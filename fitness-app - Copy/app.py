        ...
        if not saved_plans:
            ...
            for idx, item in enumerate(saved_plans):
                ...
                with st.expander(f"Plan created on {item['created_at']}"):
                    ...
                    with col_del_2:
                        # Unique key is required for buttons in loops
                    ...
                    if isinstance(plan, list):
                        ...
                        for day in plan:
                            day_name = day.get('day', 'Day').capitalize()
                            day_type = day.get('day_type', '').capitalize()
                            plan_html += f"<h3>{day_name} ({day_type})</h3><ul>"
                            
                            for ex in day['exercises']:
                                if isinstance(ex, dict):
                                    details = f"Sets: {ex.get('sets','-')}, Reps: {ex.get('reps', ex.get('time','-'))}"
                                    plan_html += f"<li><strong>{ex['name']}</strong> - <span style='font-size:0.9em'>{details}</span></li>"
                                else:
                                    plan_html += f"<li>{ex}</li>"
                            plan_html += "</ul><hr style='margin: 10px 0;'>"
                        plan_html += "</div>"
                        ...
                        for meal in plan.get("meals", []):
                            ...
                            for food in meal.get("food_items", []):
                                ...
                                plan_html += f"<li><strong>{item}</strong> ({qty})</li>"
                            plan_html += "</ul><br>"
                        plan_html += "</div>"
